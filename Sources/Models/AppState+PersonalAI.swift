...
extension AppState {
    ...
    func generateLifelogSummary(forceRefresh: Bool = false, notifyLiveActivity: Bool = true) async {
        ...
        let prompt = """
        あなたはユーザーの一日の活動を把握しているアシスタントです。以下のデータをもとに、「今日がどんな一日だったか」を日本語でまとめてください。
        ルール:
        - 挨拶・前置き不要。2〜4文に収める。
        - 1文目は必ず「どんな一日だったか」の意訳・総括にする（例:「開発に集中しつつ、夕方は外に出てリフレッシュした一日」）。数値やアプリ名の羅列で始めない。
        - 2文目以降で根拠となる出来事を簡潔に添える。データの列挙ではなく、流れが伝わる地の文で。
