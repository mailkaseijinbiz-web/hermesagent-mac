...
actor APNsSender {
    ...
    /// Push an ActivityKit content-state update to devices that registered a Live Activity push token.
    @discardableResult
    func sendLiveActivityUpdate(to tokens: [String], contentState: [String: Any], config: Config) async -> [String] {
        ...
        for token in tokens {
            ...
            do {
                ...
                if let http = response as? HTTPURLResponse, http.statusCode != 200 {
                    let msg = String(data: data, encoding: .utf8) ?? ""
                    print("[APNs] liveactivity \(http.statusCode) for token \(token.prefix(8))…: \(msg)")
                    if http.statusCode == 410 || msg.contains("Unregistered") || msg.contains("BadDeviceToken") {
                        invalidTokens.append(token)
                    }
                }
            } catch {
                print("[APNs] liveactivity send error: \(error)")
            }
        }
        return invalidTokens
    }
    ...
    /// Push-to-start a lifelog glance Live Activity.
    @discardableResult
    func sendLifeLogLiveActivityStart(
        to tokens: [String],
        headline: String,
        detail: String,
        statusLabel: String,
        ...
        let payload = LiveActivityPushPayload.lifeLogStart(
            headline: headline, detail: detail, statusLabel: statusLabel
        )
        guard let bodyData = try? JSONSerialization.data(withJSONObject: payload) else { return [] }
        ...
        for token in tokens {
            ...
            do {
                ...
                if let http = response as? HTTPURLResponse, http.statusCode != 200 {
                    let msg = String(data: data, encoding: .utf8) ?? ""
                    print("[APNs] lifelog liveactivity start \(http.statusCode) for token \(token.prefix(8))…: \(msg)")
                    if http.statusCode == 410 || msg.contains("Unregistered") || msg.contains("BadDeviceToken") {
                        invalidTokens.append(token)
                    }
                }
            } catch {
                print("[APNs] lifelog liveactivity start send error: \(error)")
            }
        }
        return invalidTokens
    }

    /// Update an active lifelog Live Activity.
    @discardableResult
    func sendLifeLogLiveActivityUpdate(
        to tokens: [String],
        headline: String,
        detail: String,
        statusLabel: String,
        ...
        let host = config.useSandbox ? "api.sandbox.push.apple.com" : "api.push.apple.com"
        let topic = "\(config.bundleId).push-type.liveactivity"
        let payload = LiveActivityPushPayload.lifeLogUpdate(
            headline: headline, detail: detail, statusLabel: statusLabel
        )
        guard let bodyData = try? JSONSerialization.data(withJSONObject: payload) else { return [] }
        ...
        for token in tokens {
            ...
            do {
                ...
                if let http = response as? HTTPURLResponse, http.statusCode != 200 {
                    let msg = String(data: data, encoding: .utf8) ?? ""
                    print("[APNs] lifelog liveactivity update \(http.statusCode) for token \(token.prefix(8))…: \(msg)")
                    if http.statusCode == 410 || msg.contains("Unregistered") || msg.contains("BadDeviceToken") {
                        invalidTokens.append(token)
                    }
                }
            } catch {
                print("[APNs] lifelog liveactivity update send error: \(error)")
            }
        }
        return invalidTokens
    }
