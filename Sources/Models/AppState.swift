...
class AppState: ObservableObject {
    ...
    func refreshFromStore() async {
        ...
        if let sid = currentSessionId, !isStreaming {
            ...
            if !stored.isEmpty {
                ...
                // Delegated turns live in the specialist's session, not this one — preserve
                // them IN PLACE (in-memory only) so the manager's view keeps the team's
                // replies in chronological order, syncing the rest from the store.
                let isDelegated: (Message) -> Bool = {
                    $0.delegatedName != nil || ($0.role == .user && $0.content.hasPrefix("［委譲→"))
                }
                if messages.contains(where: isDelegated) {
                    var merged: [Message] = []
                    var si = 0
                    for m in messages {
                        if isDelegated(m) {
    ...
    func handleSelectSession(_ session: Session) {
        ...
        // not whatever ACP was last on. Browsing raw sessions also detaches the active employee.
        ...
        self.activeProcess = HermesCLI.shared.streamPrompt(
            ...
            onEnd: { [weak self] code in
                ...
                DispatchQueue.main.async {
                    ...
                    if let idx = self.messages.firstIndex(where: { $0.id == assistantId }) {
                        if cleaned.isEmpty {
                            // Only noise/empty → drop the placeholder; store-sync fills the real reply.
    ...
    private func sendViaACP(assistantId: UUID, prompt: String, imagePath: String?) {
        Task { [weak self] in
            ...
            if let idx = self.messages.firstIndex(where: { $0.id == assistantId }) {
                if acc.isEmpty && !ok {
                    self.messages[idx].content = "応答に失敗しました（ACP）。"
    ...
    /// Fire (remove) an employee and any cached avatar.
    func fireEmployee(_ id: String) {
        if let emp = employees.first(where: { $0.id == id }), let p = emp.avatarImagePath {
    ...
    func delegate(to employeeId: String, task: String) async {
        ...
        if useACPTransport {
            let (_, ok) = await ACPClient.shared.prompt(
                wrapped, cwd: cwd,
    ...
    func handleCreateCronJob() async {
        ...
        let prompt = newCronPrompt.trimmingCharacters(in: .whitespacesAndNewlines)
